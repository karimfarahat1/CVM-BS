...
vector<int> rank_transform(vector<double> input)
{
  // create an empty ordered map
...
double cp_signif(NumericMatrix knots, NumericMatrix beta, NumericVector intercepts, NumericVector q, double x, double m)
{
  arma::mat knots_arma = as<arma::mat>(knots);
  arma::mat beta_arma = as<arma::mat>(beta);
  std::vector<double> qs = as<std::vector<double>>(q);
  std::vector<double> intercept_vec = as<std::vector<double>>(intercepts);
  
  return qreg_cdf(knots_arma, beta_arma, intercept_vec, qs, x, m);
}


pair<double,int> max_cost_cp(const vector<int> time_series)
{
  double cp_index = 2; 
  double cp_cost = 0;
...
bool fdr_control(std::map<double, int> changepoints, double fdr, double new_signif)
{
  ...
  for(std::map<double, int>::iterator it = changepoints.begin(); it != changepoints.end(); it++)
  {
    pval = (*it).first;
    count++;
    if(pval > (count / denom) * fdr){return false;}
  }
...
void CVM_BinSeg(const vector<int> time_series, 
                std::map<double, int> &changepoints,
                const int changepoint_offset,
                const double fdr,
                const NumericMatrix knots, 
                const NumericMatrix beta, 
                const NumericVector intercepts, 
                const NumericVector q)
{
  pair<double, int> cp_vec = max_cost_cp(time_series);
  cp_vec.first = 1 - cp_signif(knots, beta, intercepts, q, cp_vec.first, time_series.size());
  ...
  if(fdr_control(changepoints, fdr, cp_vec.first))
  {
    ...
    {
    vector<int> left_series = vector<int>(time_series.begin(), time_series.begin() + cp_vec.second - changepoint_offset);
    ...
    {
      vector<int> right_series = vector<int>(time_series.begin() + cp_vec.second - changepoint_offset, time_series.end());
...
std::map<double, int> CVM_BS(vector<double> input_time_series, 
                     double fdr,                                 
                     NumericMatrix knots, 
                     NumericMatrix beta, 
                     ...
                     NumericVector q)
{
  vector<int> time_series = rank_transform(input_time_series);
  std::map<double, int> changepoints;
  
  CVM_BinSeg(time_series, changepoints, 0, fdr, knots, beta, intercepts, q);
